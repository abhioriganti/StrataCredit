...
def classification_metrics(y_true: np.ndarray, probability: np.ndarray, weights: np.ndarray | None = None) -> dict[str, float]:
    ...
    if len(np.unique(y)) < 2:
        return result | {"roc_auc": float("nan"), "pr_auc": float("nan"), "top_decile_capture_rate": float("nan")}
    ...
    positives = y.sum()
    result["top_decile_capture_rate"] = float(y[p >= cutoff].sum() / positives) if positives else float("nan")
    return result


def portfolio_backtest(df: pl.DataFrame, target: str, score: str, dimensions: list[str]) -> pl.DataFrame:
