...
def _dataset_for_vintages(panel: pl.DataFrame, start: int, end: int) -> pl.DataFrame:
    return build_training_dataset(
        panel, start, end, max_scoring_dates=12
    ).drop_nulls(TARGETS)


def _fit_and_score(train: pl.DataFrame, test: pl.DataFrame, target: str) -> tuple[Pipeline, np.ndarray, list[str]]:
    train, test = encode_categoricals(train), encode_categoricals(test)
    features = sorted(set(get_feature_columns(train, list(TARGETS))) & set(get_feature_columns(test, list(TARGETS))))
    if not features:
...
def run() -> dict[str, dict[str, float]]:
    ...
    if train.is_empty() or test.is_empty():
        raise ValueError("No labeled temporal snapshots. Ensure at least 12 future reporting months are available.")
    ...
    for target in TARGETS:
        ...
        metrics[target] = classification_metrics(test[target].to_numpy(), probability, test["current_upb"].to_numpy() if "current_upb" in test.columns else None)
    ...
    with get_connection("gold") as conn:
        ...
        conn.execute("CREATE OR REPLACE TABLE gold.gold_model_scores AS SELECT * FROM model_scores_frame")
        for target in TARGETS:
            cohort = portfolio_backtest(scored, target, f"predicted_{target}", ["origination_year", "scoring_date"])
            conn.register("cohort_frame", cohort.to_pandas())
            conn.execute(f"CREATE OR REPLACE TABLE gold.gold_{target}_backtest AS SELECT * FROM cohort_frame")
    console.print(
