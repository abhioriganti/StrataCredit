...
FEATURES = ["fico", "original_ltv", "original_cltv", "dti", "loan_age", "original_upb", "mortgage_insurance_pct"]
...
def run() -> dict[str, float]:
    ...
    model = Pipeline([("imputer", SimpleImputer(strategy="median")), ("scale", StandardScaler()), ("model", Ridge(alpha=1.0))])
    ...
    metrics = {"model_name": "ridge_conditional_severity", "events_train": float(len(train)), "events_test": float(len(test)), "mae": float(np.mean(np.abs(y - prediction))), "rmse": float(np.sqrt(np.mean((y - prediction) ** 2))), "bias": float(np.mean(prediction - y)), "balance_weighted_mae": float(np.average(np.abs(y - prediction), weights=weights))}
    ARTIFACT.parent.mkdir(parents=True, exist_ok=True); joblib.dump({"features": FEATURES, "model": model}, ARTIFACT)
    with get_connection("gold") as conn:
        conn.register("severity_metrics", pd.DataFrame([metrics])); conn.execute("CREATE OR REPLACE TABLE gold.gold_loss_severity_model_metrics AS SELECT * FROM severity_metrics"); conn.unregister("severity_metrics")
    return metrics
