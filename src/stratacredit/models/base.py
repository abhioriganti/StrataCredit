...
class SeverityMetrics:
    ...
    rmse: float
    bias: float
    balance_weighted_mae: Optional[float] = None
    split: str = ""
...
def evaluate_classification(
    ...
    return ClassificationMetrics(
        roc_auc=roc, pr_auc=prauc, brier_score=brier,
        log_loss_val=ll, top_decile_capture=tdc,
        n_positive=int(y_true.sum()), n_total=len(y_true), split=split,
    )
...
def evaluate_severity(
    ...
    return SeverityMetrics(mae=mae, rmse=rmse, bias=bias,
                           balance_weighted_mae=wa_mae, split=split)
