...
def run() -> dict[str, float]:
    ...
    x_train = train[FEATURES].fillna(medians).to_numpy(dtype=np.float32)
    x_test = test[FEATURES].fillna(medians).to_numpy(dtype=np.float32)
    ...
    metrics = {
        "model_name": "xgboost_aft_prepayment",
    ...
    joblib.dump(
        {"features": FEATURES, "model": model}, "artifacts/models/survival_prepay_aft.joblib"
    )
    with get_connection("gold") as conn:
        conn.register("survival_metrics", pd.DataFrame([metrics]))
        conn.execute(
            "CREATE OR REPLACE TABLE gold.gold_survival_prepay_metrics AS SELECT * FROM survival_metrics"
        )
